...
const links = [
    ...
    { href: "#profile", text: "Profile", icon: "#profile" }
];
...
<header>
    <nav>
        <div class="link-background" style="transform: translateX({128.25 * activeLinkIndex}%)"></div>
        <ul>
            {#each links as link, index}
                <li>
                    <a href={link.href} class:active={index === activeLinkIndex} on:click={() => handleClick(index)}>
                        <svg>
